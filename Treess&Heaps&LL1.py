...
class BinaryTree: # insert logic same as the functional implementation
    ...
    def insertLeft(self,newNode):
        if self.leftChild == None:
            self.leftChild = BinaryTree(newNode) # this can be a TreeNode as well (Need to test if the ecursion is unnecessary here)
...
class BinarySearchTree:
    ...
    def get(self, key):
        if self.root:
            ...
            return None
    # __setitem__ and __getitem() explained
    '''
        ...
        # Example usage
        obj = CustomDict()
        obj['key1'] = 'value1'  # Calls __setitem__
        print(obj['key1'])  # Calls __getitem__
        
        # Example usage for __delittem__()
        obj['a'] = 1
        obj['b'] = 2
        print(obj.data)  # Output: {'a': 1, 'b': 2}
    ...
    def delete(self, key): # remove calls delete which is expanded down
        if self.size > 1:
    ...
    def remove(self, currentNode):# remove calls spliceout
...
class RBTreeNode:
    ...
    def __str__(self):
        return f"RBTreeNode({self.key}, {self.value}, {self.color})"
...
class RBTree:
    ...
    def fix_insert(self, node):
        # Implement the fix-up logic for red-black tree properties
...
# Exxpansive variation of LinkedList class with more methods
class LinkedList1:
    def __init__(self):
        self.head = None

    # Method to add a node at the beginning of the LL
    def insertAtBegin(self, data):# insert
        new_node = Node(data)
        new_node.next = self.head
        self.head = new_node
    ...
    def insertAtEnd(self, data): # append
        new_node = Node(data)
    ...
    def remove_at_index(self, index):# delete
        if self.head is None:
            return
